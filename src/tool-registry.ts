...
export const registerRachisTools = (server: ToolRegistrar): void => {
				...
				content: [
					{
						type: "text",
						text: JSON.stringify(
							{ versions: AVAILABLE_VERSIONS, latest: LATEST_VERSION },
							null,
							2,
						),
					},
	...
	server.tool(
		...
		async ({
			...
			try {
				const { graph } = getGraph(version);
				const actions = graph.listActions({ distribution, plugin });
				const actionStrings = normalizeActionIds(
					actions.map((a) => toActionId(a.plugin, a.action)),
				);
				return {
					content: [{ type: "text", text: JSON.stringify(actionStrings, null, 2) }],
				};
			} catch (e: any) {
				return {
					content: [{ type: "text", text: JSON.stringify({ error: e.message }) }],
	...
	server.tool(
		...
		"Lists all known Rachis semantic types. Optionally filter by distribution name.",
		{
			distribution: z
				.string()
				.optional()
				.describe("Optional distribution name to scope semantic types"),
			version: versionParam,
		},
		readOnlyQueryAnnotations,
		async ({ distribution, version }: { distribution?: string; version?: string }) => {
				...
				if (description === undefined) {
					return {
						content: [
							{
								type: "text",
								text: JSON.stringify({ error: `Type '${type_name}' not found.` }),
							},
						],
					};
				}
	...
	server.tool(
		...
		"Finds all Rachis actions that accept the provided semantic type as an input.",
		{
			...
			distribution: z
				.string()
				.optional()
				.describe("Optional distribution name to scope the search"),
		...
		async ({
			...
			try {
				const { graph } = getGraph(version);
				const compatible = graph.findCompatibleActions(semantic_type, {
					distribution,
					plugin,
				});
				return {
	...
	server.tool(
		...
		"Maps a free-text description of an input kind (for example 'reads' or 'contigs') to likely semantic types and the actions that can consume them.",
		{
			...
			distribution: z
				.string()
				.optional()
				.describe("Optional distribution name to scope candidate discovery"),
			plugin: z
				.string()
				.optional()
				.describe("Optional plugin name to scope candidate discovery"),
			limit: z
				.number()
				.int()
				.min(1)
		...
		async ({
			...
			try {
				const { graph } = getGraph(version);
				const candidates = graph.findInputTypeCandidates(kind, {
					distribution,
					plugin,
					limit,
				});
	...
	server.tool(
		...
		"Finds actions that consume all or some of the provided artifact types as inputs.",
		{
			...
			distribution: z
				.string()
				.optional()
				.describe("Optional distribution name to scope consumer search"),
	...
	server.tool(
		...
		"Compares two Rachis schema versions and returns actions that were added, removed, or had their interface (inputs, parameters, outputs) changed. If from_version is omitted, the version immediately preceding to_version is used.",
		{
			...
			from_version: z
				...
				.optional()
				.describe(
					"The base version to compare from. Defaults to the version preceding to_version.",
				),
		},
		readOnlyQueryAnnotations,
	...
	server.tool(
		...
		"Finds actions that produce all of the specified artifact types as outputs.",
		{
			...
			distribution: z
				.string()
				.optional()
				.describe("Optional distribution name to scope producer search"),
