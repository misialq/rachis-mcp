...
const scoreTokenMatches = (
	...
	for (const token of queryTokens) {
		...
		const fuzzyMatch = [...candidateTokens].some(
			(candidateToken) =>
...
export class KnowledgeGraph {
	...
	listSemanticTypes(filters?: { distribution?: string; plugin?: string }): SemanticTypeInfo[] {
		...
		if (!filters?.distribution && !filters?.plugin) {
			return [...metadataByType.values()].sort((left, right) =>
			...
			for (const definitions of [details.inputs, details.outputs]) {
				...
				for (const definition of Object.values(definitions)) {
					...
					for (const typeExpression of typeExpressions) {
						collectSchemaBackedTypesFromExpression(
							typeExpression,
							knownTypes,
							scopedTypes,
						);
	...
	listActions(filters: {
		...
		const pluginFilter = this.resolvePluginFilter(filters);
		const actions: { plugin: string; action: string }[] = [];

		for (const { plugin, action } of this.getAllActions()) {
			if (pluginFilter && !pluginFilter.has(plugin)) continue;
			actions.push({ plugin, action });
		}
		...
		for (const { plugin, details } of this.getAllActions()) {
			...
			for (const [inputName, inputDef] of Object.entries(details.inputs)) {
				...
				for (const inputType of inputTypes) {
					...
					existing.inputNameCounts.set(
						inputName,
						(existing.inputNameCounts.get(inputName) || 0) + 1,
					);
	...
	private matchesInputType(availableType: string, inputDef: Input): boolean {
		return inputDef.type.some((requiredType) =>
	...
	findCompatibleActions(
		...
		for (const { plugin, action, details } of this.getAllActions()) {
			...
			const matchesAnyInput = actionInputs.some((inputDef) => {
				return inputDef.type.some((requiredType) =>
					this.checkCompatibility(semanticType, requiredType),
				);
			});
	...
	findInputTypeCandidates(
		...
		for (const semanticType of candidateTypes) {
			...
			const commonInputNames = [...(usage?.inputNameCounts.entries() || [])].sort(
			...
			const actionDescriptionTokens = new Set(
				actionDescriptionTexts.flatMap((actionDescription) =>
					tokenizeSearchText(actionDescription),
				),
			);
			...
			const actionDescriptionMatches = scoreTokenMatches(
				queryTokens,
				actionDescriptionTokens,
				2,
				1,
			);
			...
			for (const [inputName, count] of commonInputNames) {
				...
				if (
					...
					(normalizedInputName.includes(normalizedQuery) ||
						normalizedQuery.includes(normalizedInputName))
				) {
					score += 4;
			...
			const dedupedConsumers = Array.from(
				new Map(
					consumers.map((consumer) => [
						`${consumer.plugin}:${consumer.action}`,
						consumer,
					]),
				).values(),
			);
			...
			const rankedInputNames = commonInputNames
				.sort((left, right) => {
					const leftMatchesQuery = normalizeSearchText(left[0]).includes(normalizedQuery)
						? 1
						: 0;
					const rightMatchesQuery = normalizeSearchText(right[0]).includes(
						normalizedQuery,
					)
						? 1
						: 0;
					return (
						rightMatchesQuery - leftMatchesQuery ||
						right[1] - left[1] ||
	...
	planWorkflow(
		...
		for (let depth = 0; depth < maxDepth; depth++) {
			...
			for (const { plugin, action, details } of this.getAllActions()) {
				...
				const requiredInputs = Object.values(details.inputs).filter(
					(inputDef) => inputDef.required,
				);
				const allSatisfied = requiredInputs.every((inputDef) => {
					return inputDef.type.some((reqType) =>
						this.hasCompatibleType(availableTypes, reqType),
					);
				});
			...
			if (
				!includePreference &&
				targetTypes.every((t) => this.hasCompatibleType(availableTypes, t))
			)
				break;
		}
		...
		for (let i = 0; i < entries.length; i++) {
			...
			for (const inputDef of Object.values(details.inputs)) {
				...
				const directAcceptedTypes = acceptedTypes.filter(
					(t) => !t.trim().startsWith("List["),
				);
				const hasPlanProducer =
					directAcceptedTypes.length > 0 &&
					entries.some(
						(_, j) =>
							j !== i &&
							entryOutputTypes[j].some((t) =>
								directAcceptedTypes.some((req) => this.checkCompatibility(t, req)),
							),
					);
				if (
					!hasPlanProducer &&
					acceptedTypes.some((t) => this.hasCompatibleType(startTypes, t))
				) {
					...
					if (outputTokens.size > 0) {
						for (const acceptedType of acceptedTypes) {
							...
							if (
								innerTokens.size === 0 ||
								![...innerTokens].some((t) => outputTokens.has(t))
							)
								continue;
							let bestJ = -1;
							let bestDepth = -1;
							for (let j = 0; j < entries.length; j++) {
								...
								if (
									entryOutputTypes[j].some((t) =>
										this.checkCompatibility(t, innerType),
									)
								) {
				...
				for (let j = 0; j < entries.length; j++) {
					...
					if (
						entryOutputTypes[j].some((t) =>
							acceptedTypes.some((req) => this.checkCompatibility(t, req)),
						)
					) {
						if (entries[j].depth > bestDepth) {
							bestJ = j;
	...
	private traceBackward(
		...
		while (queue.length > 0) {
			...
			for (const inputDef of Object.values(actionDetails.inputs)) {
				...
				const planAvailable = this.computePlanAvailableTypes(
					startTypes,
					neededActions,
					currentDepth,
				);
	...
	private selectBestProducer(
		...
		let best: { plugin: string; action: string; depth: number; producedType: string } | null =
			null;
		let bestScore = -1;
		...
		for (const { plugin, action, details } of this.getAllActions()) {
			...
			if (existingEntries.length > 0) {
				...
				const needsNewInvocation = existingEntries.some(
					(entry) =>
	...
	private resolvePluginFilter(filters?: {
		distribution?: string;
		plugin?: string;
		...
		if (filters?.distribution) {
			...
			scopedPlugins = new Set(
				this.schema.distributions[distKey].plugins.map(
					(pluginName) => this.findKey(this.schema.plugins, pluginName) || pluginName,
				),
			);
		}
	...
	private outputTypesSharePort(action: Action, typeA: string, typeB: string): boolean {
		...
		for (const outputDef of Object.values(action.outputs)) {
			...
			const matchesA = outputDef.type.some(
				(t) => this.checkCompatibility(t, typeA) || this.checkCompatibility(typeA, t),
			);
			const matchesB = outputDef.type.some(
				(t) => this.checkCompatibility(t, typeB) || this.checkCompatibility(typeB, t),
			);
