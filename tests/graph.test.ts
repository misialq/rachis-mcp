...
test("findConsumers filters results by distribution", () => {
	...
	const consumers = toActionIds(
		graph.findConsumers(["TypeA"], "required_inputs", { distribution: "only_taxa" }),
	);
...
test("findConsumers filters results by plugin", () => {
	const graph = new KnowledgeGraph(schema);
	const consumers = toActionIds(
		graph.findConsumers(["TypeA"], "required_inputs", { plugin: "planner" }),
	);
...
test("findConsumers returns empty when plugin is outside selected distribution", () => {
	...
	const graph = new KnowledgeGraph(constrainedSchema);
	const consumers = toActionIds(
		graph.findConsumers(["TypeA"], "required_inputs", {
			distribution: "only_taxa",
			plugin: "planner",
		}),
	);
...
test("findConsumers throws for unknown distribution or plugin filters", () => {
	...
	assert.throws(
		() => graph.findConsumers(["TypeA"], "required_inputs", { distribution: "missing" }),
		/Distribution 'missing' not found/,
	);
	assert.throws(
		() => graph.findConsumers(["TypeA"], "required_inputs", { plugin: "missing_plugin" }),
		/Plugin 'missing_plugin' not found/,
	);
});
...
test("findProducers returns empty when plugin is outside selected distribution", () => {
	...
	const producers = toActionIds(
		constrainedGraph.findProducers(["TypeC"], { distribution: "only_taxa", plugin: "planner" }),
	);
...
test("findProducers throws for unknown distribution or plugin filters", () => {
	...
	assert.throws(
		() => graph.findProducers(["TypeC"], { distribution: "missing" }),
		/Distribution 'missing' not found/,
	);
	assert.throws(
		() => graph.findProducers(["TypeC"], { plugin: "missing_plugin" }),
		/Plugin 'missing_plugin' not found/,
	);
});
...
test("listActions lists actions in a distribution and honors plugin filter", () => {
	const graph = new KnowledgeGraph(schema);

	const allTestActions = toActionIds(graph.listActions({ distribution: "test" }));
	assert.deepEqual(allTestActions.sort(), [
		"assembly:assemble_c",
		"planner:combine",
		"planner:convert",
		"planner:derive_b",
		"planner:metadata_splitter",
		"planner:property_consumer",
		"taxa:profile",
	]);

	const plannerTestActions = toActionIds(
		graph.listActions({ distribution: "test", plugin: "planner" }),
	);
	assert.deepEqual(plannerTestActions.sort(), [
		"planner:combine",
		"planner:convert",
		"planner:derive_b",
		"planner:metadata_splitter",
	...
	assert.throws(
		() => graph.listActions({ distribution: "missing" }),
		/Distribution 'missing' not found/,
	);
	assert.throws(
		() => graph.listActions({ distribution: "test", plugin: "missing" }),
